...
- **`MonoCloudCodedException`** sits between `MonoCloudRequestException` and the statuses the apis report an `error_code` with — 400, 402, 403, 404, 409 — and is the only place `ErrorCode` is exposed. The rest (401, 422, 429, 500) derive from `MonoCloudRequestException` directly and deliberately have no `ErrorCode`: nothing server-side sets one for them, so offering the member would only ever return null. Adding a status to the coded set means reparenting its exception here, not moving the property up.
- **`MonoCloudClientBase.ThrowProblem`** (protected virtual) is the seam for an sdk whose api reports errors this status mapping can't express. Such an sdk overrides it, throws its own exception for the codes it declares, and calls `base.ThrowProblem` for everything else. What it throws must derive from `MonoCloudException`, and the status hierarchy stays here rather than being reimplemented per sdk, so an sdk-specific 404 should derive from `MonoCloudNotFoundException` and keep that type check holding. `ProcessRequestAsync` throws a generic `MonoCloudException` for the status if an override returns without throwing.
